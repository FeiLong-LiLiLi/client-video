...
// 将$resource服务当做自定义服务的基础。本身是一个创建资源对象的工厂，
// 返回的$resource对象中包含了同后端服务器进行的交互的高层API.
   
...
// 指定query方法，通过传递一个对象来配置这个方法,配置项中需要指定的内容有：请求的类型（GET、POST等）、以及返回的结果是否是一个数组
// 调用contributors.query()
// 根据上面的代码，对于配置对象{query: {method: 'GET', isArray: false}，Angular会将其解析成一个方法，然后把这个方法绑定到返回的Restful对象上
//就是contributors.query=function（isArray）
...
app.controller('DashController', function ($scope, sources, contributors, dashifTestVectors) {
    ...
    $scope.onChartEnableButtonClick = function () {
        ...
        // fadeto 是逐渐变得没有颜色，用在图形disable按钮中，5秒，变0.3透明
    };
    ...
    $scope.toggleOptionsGutter = function (bool) {
     ...
     }; //点击$scope.optionsGutter =!optionsGutter为ture
    ...
    $scope.doLoad = function () {
        ...
        clearPlay();    //清楚上一次点击播放,防止连续点击播放
        clearVideo();   //清除原有的播放,防止提交不观看的视频信息
        clickPlayVideo();   //开始播放视频
    ...
    // custom right  copy URL function
    $scope.copyUrl = function () {
        var menu = document.getElementById('rightclickmenu');
        menu.style.visibility = 'hidden';

        var Urlv=$scope.selectedItem.url;
    ...
    // alert('复制成功');不然会点了这个才会menu才消失
        }


// video information panel show
    $scope.videoPanel = function () {
        var menu = document.getElementById('rightclickmenu');
        menu.style.visibility = 'hidden';

        var videoMess= document.getElementById('videoMessage');
        videoMess.style.visibility = "visible";

         } 

    $scope.closeMessage = function () {
        var videoMess= document.getElementById('videoMessage');
        videoMess.style.visibility = "hidden";
    }
    ...
    //自己加的
    $scope.hideChartData = function () {
        $scope.chartHide = !$scope.chartHide;
       $('#chart-inventory').fadeTo(500, $scope.chartHide ? 1 : 0);
    };
...
//封装请求
function ajaxReq(method, url, val) { 
    var params = '', res;
    for(let obj in val){
        params += obj+"="+val[obj]+"&";
    }
...
function addPlay(){
    ...
    xmlhttp.open("GET","http://localhost:8004/api/info/today/play/add?today=" +today,true);
    xmlhttp.send();
}
...
function getvideoInfo(){
    let videoId = Math.floor(Math.random() * 1000 + 1); 
    let videoName = 'videoName'+videoId;
    let videoUrl = $(".vurl").text();
    let videoDuration = $("#videoDuration").text();
    return {
        videoId: videoId,
        videoName: videoName,
        videoUrl: videoUrl,
...
function getVideoPlayInfo(){
    ...
    let videoBitrateDownloading = $(".vbitrate").text();
    let videoBufferLength = $(".vbuffer").text(); 
    let videoDroppedFrames = $(".vdframes").text();
    let videoRatio = $(".vratio").text();
    let audioBitrateDownloading = $(".abitrate").text();
    let audioBufferLength = $(".abuffer").text();
    let audioDroppedFrames= $(".adframes").text();
    let audioRatio = $(".aratio").text();          
    let videoTime = $("#videoTime").text();
    return {   
        startDate: startDate,
        startTime: startTime,
        videoBitrateDownloading: videoBitrateDownloading,
        videoBufferLength: videoBufferLength,
        videoDroppedFrames: videoDroppedFrames,
        videoRatio: videoRatio,
        audioBitrateDownloading: audioBitrateDownloading,
...
function userPlay(){
    ...
    let videoPlayInfo = getVideoPlayInfo();
    let data ={
        startDate: videoPlayInfo.startDate,
        ...
        startTime: videoPlayInfo.startTime
    }
    // console.log(data);
    ajaxReq('POST','http://localhost:8004/api/info/today/user/play/add', data);
    monitorVideo(userInfo,videoInfo);

}

var isClick = false, clickPlay;
//延迟3秒播放,isClick是防止在3s内重复播放
function clickPlayVideo(){
    if(isClick == false){
        clickPlay = setTimeout(function(){
            isClick = true;
            addPlay();
            userPlay();
            setTimeout(function(){isClick = false}, 5000)
        },5000);
    } 
}
//清除点击播放视频
function clearPlay(){
    clearTimeout(clickPlay)
}


var playVideo;
//监听视频发送数据,20秒一次
function monitorVideo(userInfo,videoInfo){
    playVideo = setInterval(function () { 
        let videoPlayInfo = getVideoPlayInfo();
        let data = { 
            userId: userInfo.userId,
            userName: userInfo.userName,
            ...
            videoName: videoInfo.videoName,
            videoUrl: videoInfo.videoUrl,
            videoBitrateDownloading: videoPlayInfo.videoBitrateDownloading,
            videoBufferLength: videoPlayInfo.videoBufferLength,
            videoDroppedFrames: videoPlayInfo.videoDroppedFrames,
            videoRatio: videoPlayInfo.videoRatio,
            audioBitrateDownloading: videoPlayInfo.audioBitrateDownloading,
            audioBufferLength: videoPlayInfo.audioBufferLength,
            audioDroppedFrames: videoPlayInfo.audioDroppedFrames,
            audioRatio: videoPlayInfo.audioRatio,
            videoTime: videoPlayInfo.videoTime,
            nowTime: videoPlayInfo.startTime,
        };
        // console.log(data)
        ajaxReq('POST', 'http://localhost:8004/api/monitor/now/play/info', data);
    },20000);
}
...
function clearVideo(){
    ...
    //                 videoName: videoInfo.videoName,
    //                 videoUrl: videoInfo.videoUrl,
    //                 videoBitrateDownloading: videoInfo.videoBitrateDownloading,
    //                 videoBufferLength: videoInfo.videoBufferLength,
    //                 videoDroppedFrames: videoInfo.videoDroppedFrames,
    //                 videoRatio: videoInfo.videoRatio,
    //                 audioBitrateDownloading: videoInfo.audioBitrateDownloading,
    //                 audioBufferLength: videoInfo.audioBufferLength,
    //                 audioDroppedFrames: videoInfo.audioDroppedFrames,
    //                 audioRatio: videoInfo.audioRatio,
    //                 videoTime: videoInfo.videoTime,
    //                 nowTime: videoInfo.startTime,
    //             };
    //             console.log(data)
    //             // ajaxReq('POST', 'http://localhost:8004/api/monitor/now/play/info', data);
    //         },5000);
